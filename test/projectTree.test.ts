...
describe('ProjectTreeProvider', () => {
  ...
  it('dispatches renameIfActive when a refresh detects a windowName change for a registered terminal', async () => {
    const tmux = {
      listSessions: vi.fn(async () => [
        { sessionName: 'wt-_work_alpha-main__term-1', windowName: 'claude' },
      ]),
  ...
  it('does not dispatch renameIfActive when a refresh shows no windowName change', async () => {
    const tmux = {
      listSessions: vi.fn(async () => [
        { sessionName: 'wt-_work_alpha-main__term-1', windowName: 'zsh' },
        { sessionName: 'wt-_work_alpha-main__term-2', windowName: 'claude' },
    ...
    // term-1 unchanged → no rename. term-2 newly seen, so it gets a
    // rename attempt (registry.renameIfActive no-ops if not registered).
    expect(renameIfActive).not.toHaveBeenCalledWith('wt-_work_alpha-main__term-1', expect.anything());
  });
  ...
  it('handleActiveTerminalChange renames to the cached label and refreshes', () => {
    ...
    const worktreeListCache = {
      get: vi.fn(() => [
        { path: '/work/alpha-main', head: 'a', branch: 'main', bare: false, detached: false },
      ]),
      set: vi.fn(async () => undefined),
    } as unknown as WorktreeListCacheStore;
    const terminalSessionListCache = {
