...
export function renderAgentHookScript(
  ...
  return [
    ...
    '    printf \'{"status":"%s","statusAt":%s,"agent":"%s","pid":%s,"startTime":"%s","message":"%s"}\\n\' "$1" "$status_at" "$agent" "$agent_pid" "$escaped_start_time" "$escaped_message" > "$tmp" || { rm -f "$tmp"; return 0; }',
    '  else',
    '    printf \'{"status":"%s","statusAt":%s,"agent":"%s","pid":%s,"startTime":"%s"}\\n\' "$1" "$status_at" "$agent" "$agent_pid" "$escaped_start_time" > "$tmp" || { rm -f "$tmp"; return 0; }',
    '  fi',
