...
| **DetachedOpen** | The opt-in sibling of SwitchOperation: open a Worktree in a new VS Code window via `vscode.openFolder(uri, { forceNewWindow: true })` without touching the current window. Does **not** mutate `ActiveWorktree` (no single "active" slot when multiple windows are alive). Sets `FocusIntent` so the new window lands on the Deck Activity Bar. The escape hatch users reach for when ADR-0003's reload is too costly to pay for the current switch. See [ADR-0004](./docs/adr/0004-open-worktree-in-new-window.md). |
