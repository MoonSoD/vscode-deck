...
2. **Add Project still auto-reloaded.** `projectTree.ts:127` called
   ...
   projects from cold paid three reloads.
...
Storage was the fourth misalignment: `deck.projects` lived in the user's
...
2. **Activation.** `activationEvents` becomes `["onView:deck.projects"]`.
...
3. **AddProject decoupled.** AddProject becomes pure registration: append to
   the new `ProjectRegistryStore`, refresh the tree, auto-expand and
   `TreeView.reveal` the new Project node, then show a non-modal toast
   `Added project <basename>.` with `Switch` / `Open in New Window` actions
...
4. **FocusIntent retired.** All three set-sites (SwitchOperation, AddProject,
...
5. **Storage migrated.** Registered projects move from `settings.json`
   (`deck.projects` config) to a new `ProjectRegistryStore` in `globalState`.
   On first activation of v0.0.10+, a one-shot migration copies any existing
   `deck.projects` entries into globalState and clears the settings entry.
   The `contributes.configuration.deck.projects` schema is removed from
   `package.json`. Add Project becomes the only entry point.
...
6. **First-install discovery.** A `contributes.walkthroughs` entry with a
   ...
   `viewsWelcome` empty-state ("No projects yet. [Add Project]") takes over.
...
- ProjectRegistryStore mirrors WorktreeOrderStore: `globalState`-backed, list
...
- Post-AddProject reveal requires holding a `TreeView` handle in
  `extension.ts` and injecting it into `ProjectTreeProvider`. One additional
  constructor dependency.
- The AddProject toast reuses `WorktreeSwitcher` for Switch and
  `DetachedOpener` for Open in New Window — same call sites as the
  ADR-0005 toast, different source path (the seed path of the new Project).
...
- Add Project is no longer destructive-in-place. N projects can be registered
...
- One UX regression: the first time a user opens (via AddProject's `Switch`,
...
- `ActiveWorktreeStore` shrinks: `setFocusIntent` / `consumeFocusIntent` are
  ...
  hygiene path and clicks on Project nodes that re-open their last worktree.
...
- ADR-0005 (post-create switch is opt-in). The same opt-in-switch posture is
  extended to AddProject — both create flows now end identically: refresh,
